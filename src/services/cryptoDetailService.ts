...
// 김치프리미엄 차트 응답 타입 (새로 추가)
export interface KimchiPremiumChartResponse {
  symbol: string;
  timestamp: string;
...
export class CryptoDetailService {
   ...
   * 
   * 왜 별도 메서드로 분리했는가:
   * - 차트 데이터는 시각화 목적으로만 사용되므로 분리
   * - 시간 범위를 파라미터로 받아 유연한 조회 지원
   * - 향후 캐싱이나 실시간 업데이트 기능 추가 용이
   */
  async fetchKimchiPremiumChart(
    symbol: string, 
    days: number = 7
  ): Promise<KimchiPremiumChartResponse> {
    try {
      // 유효성 검증: 최대 30일까지만 허용
      const validDays = Math.min(Math.max(days, 1), 30);
      
      return await this.apiClient.get<KimchiPremiumChartResponse>(
        `/kimchi-premium/${symbol}/chart?days=${validDays}`
      );
    } catch (error) {
